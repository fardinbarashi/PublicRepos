...
if ($runPowerShellA) {
    ...
    $containerParams = @{
        ...
        Environment = @(
            ...
            'SQLSERVER_PASSWORD=TypeInSomeKindOfPassWord'
            ...
            'ORACLE_PASSWORD=TypeInSomeKindOfPassWord'
            ...
            'MYSQL_PASSWORD=TypeInSomeKindOfPassWord'
            ...
            'POSTGRESQL_PASSWORD=TypeInSomeKindOfPassWord'
        )
        Interactive = $true
    }
...
if ($runPowerShellB) {
    ...
    $containerParams = @{
        ...
        Environment = @(
            ...
            'DB2_PASSWORD=TypeInSomeKindOfPassWord'
            ...
            'INFORMIX_PASSWORD=TypeInSomeKindOfPassWord'
            )
        Interactive = $true
    }
    Write-LogMessage -Message "Starting setup of container $($containerParams.Name)"
...
CREATE LOGIN StackOverflow WITH PASSWORD = 'TypeInSomeKindOfPassWord', CHECK_POLICY = OFF
GO
...
CREATE USER stackoverflow IDENTIFIED BY TypeInSomeKindOfPassWord DEFAULT TABLESPACE users QUOTA UNLIMITED ON users TEMPORARY TABLESPACE temp;
...
sqlplus / as sysdba <<END_OF_SQL
ALTER SESSION SET CONTAINER=XEPDB1;
CREATE USER geodemo IDENTIFIED BY TypeInSomeKindOfPassWord DEFAULT TABLESPACE users QUOTA UNLIMITED ON users TEMPORARY TABLESPACE temp;
...
if ('MySQL' -in $DBMS) { 
    $containerParams = @{
        ...
        Environment = @(
            'MYSQL_ROOT_PASSWORD=TypeInPassWord'
        )
    }
    Write-LogMessage -Message "Starting setup of container $($containerParams.Name)"
    if (Get-MyDockerContainer -Name $containerParams.Name) {
    ...
    $null = Invoke-MyDockerContainer -Name $containerParams.Name -Shell bash -Command @'
mysql -p'TypeInPassWord' <<END_OF_SQL
CREATE USER 'stackoverflow'@'%' IDENTIFIED BY 'TypeInSomeKindOfPassWord';
CREATE DATABASE stackoverflow;
...
if ('MariaDB' -in $DBMS) { 
    $containerParams = @{
        ...
        Environment = @(
            'MARIADB_ROOT_PASSWORD=TypeInPassWord'
        )
    }
    Write-LogMessage -Message "Starting setup of container $($containerParams.Name)"
    if (Get-MyDockerContainer -Name $containerParams.Name) {
    ...
    Write-LogMessage -Message "Creating user and database"
    $null = Invoke-MyDockerContainer -Name $containerParams.Name -Shell bash -Command @'
mariadb -p'TypeInPassWord' <<END_OF_SQL
CREATE USER 'stackoverflow'@'%' IDENTIFIED BY 'TypeInSomeKindOfPassWord';
CREATE DATABASE stackoverflow;
GRANT ALL PRIVILEGES ON stackoverflow.* TO 'stackoverflow'@'%';
exit
...
if ('PostgreSQL' -in $DBMS) { 
    $containerParams = @{
        ...
        Environment = @(
            'POSTGRES_PASSWORD=TypeInPassWord'
        )
    }
    Write-LogMessage -Message "Starting setup of container $($containerParams.Name)"
    if (Get-MyDockerContainer -Name $containerParams.Name) {
...
psql <<END_OF_SQL
CREATE USER stackoverflow WITH PASSWORD 'TypeInSomeKindOfPassWord';
...
su - postgres <<END_OF_SHELL
psql <<END_OF_SQL
CREATE USER geodemo WITH PASSWORD 'TypeInSomeKindOfPassWord';
...
7            '50000:50000'
        ...
        Environment = @(
            'LICENSE=accept'
            'DB2INST1_PASSWORD=TypeInPassWord'
            'DBNAME=DEMO'
        )
        Privileged  = $true
    }
...
echo 'stackoverflow:TypeInSomeKindOfPassWord' | chpasswd
'@
...
sudo su -c 'echo 'stackoverflow:TypeInSomeKindOfPassWord' | chpasswd'
. ./.bashrc
...
CREATE ROLE stackoverflow WITH PASSWORD = 'TypeInSomeKindOfPassWord' AND LOGIN = true;
