<#
Get out users from MIM
Check if a user Fname is longer then 20
Check if a user Lname is longer then 30 
If longer add to a text file
   ...
   $line = ""
    foreach($Attribute in $Attributes.Split('|')) { # Start foreach($Attribute in $Attributes
        switch ($Attribute) { # Start Switch    
         ...
         "FirstName" 
          { # Start FirstName
           $line += $Person.QualityAssuredFirstNames
          } # End First Name
         "LastName" 
          { # Start LastName
           $line += "$($Person.QualityAssuredLastName)"
          } # End Last Name
         "EmailAdress" 
          { # Start Email Adress 
             $line += $Person.Email
...
# Section 2 : Take Every Users that have Firstname longer then 20 char or Lastname longer than 30 char and add to file $TruncateFileCreateUpdateUSers 
Try
 { # Start Try
  Get-Date -Format "yyyy/MM/dd HH:mm:ss"
  $WhatDoesThisSectionDo = "Step 2 : Take Every Users that have Firstname longer then 20 char or Lastname longer than 30 char and add to file TruncateFiles "
  Write-Host $WhatDoesThisSectionDo "0%" -ForegroundColor Yellow
