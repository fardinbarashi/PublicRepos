<#
 ...
 News : Added Long Int 2023-02-07
#>
       ...
       If(Test-Path -Path $VhdxImage.FullName -PathType Leaf)
        ...
        if ($CheckMountedVhdx.Attached) 
         { # Start if ($CheckMountedVhdx.Attached) 
          ...
          # Custom Object
          New-Item -Path "$PSScriptRoot\DiskPartConfig\$LogFileDate\" -ItemType "directory" -Force
          ...
          { # Start Foreach ( $MountedVhdx In $CheckMountedVhdx) 
           $GetVolume | ForEach-Object {
            ...
            $Size = [long]$Size
            $SizeRemaining = [long]$SizeRemaining
         ...
         Dismount-VHD -Path $VhdxImage.FullName -Verbose
